...
def math_equal(
    ...
    if (
        ...
        if len(pred_parts) == len(ref_parts):
            if all(
                [
                    math_equal(
                        pred_parts[i], ref_parts[i], include_percentage, is_close, timeout
                    )
    ...
    if prediction.count("=") == 1 and reference.count("=") == 1:
        ...
        if math_equal(
            prediction.split("=")[1], reference, include_percentage, is_close, timeout
        ):
            return True
    elif (
        reference.count("=") == 1
        ...
        if math_equal(
            prediction, reference.split("=")[1], include_percentage, is_close, timeout
        ):
            return True

    # symbolic equal with sympy
    if timeout:
        # Use a generous timeout (60s) to allow complex but valid expressions to complete
        # Only truly pathological cases will timeout
        if call_with_timeout(symbolic_equal_process, prediction, reference, timeout=60):
            return True
    else:
