...
class PythonExecutor:
    def __init__(
        ...
        # Removed unused process pool to avoid zombie processes
        # self.pool = Pool(min(16, multiprocess.cpu_count()))
    ...
    def batch_apply(self, batch_code):
        ...
        pool = None
        try:
            # Use try-finally to ensure the process pool is always cleaned up
            # with ProcessPool(max_workers=min(len(all_code_snippets), os.cpu_count())) as pool:
            pool = ProcessPool(max_workers=min(len(all_code_snippets), 1))
            executor = partial(
                self.execute,
            ...
            while True:
                try:
                    ...
                    # Do not call exit() directly; it prevents ProcessPool from cleaning up properly
                    print(f"Exception during execution: {error}")
                    import traceback
                    traceback.print_exc()
            ...
            # Ensure the process pool is always properly closed
            if pool is not None:
                pool.close()
                pool.join(timeout=10)  # wait for all processes to finish, up to 10 seconds
