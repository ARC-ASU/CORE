...
"""vLLM Qwen3 model support adapted from the upstream Qwen2 implementation."""
...
class Qwen3ForCausalLM(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(
        ...
        if (cache_config is not None and cache_config.sliding_window is not None
                and getattr(config, "max_window_layers",
                            config.num_hidden_layers) <
                config.num_hidden_layers):
            raise ValueError("Sliding window for some but all layers is not "
                             "supported. This model uses sliding window "
                             f"but `max_window_layers` = {config.max_window_layers} "
                             f"is less than `num_hidden_layers` = {config.num_hidden_layers}. "
...
def register_qwen3_model() -> None:
    """Register Qwen3ForCausalLM with vLLM's model registry."""
    from vllm.model_executor.models.registry import ModelRegistry

    try:
        ModelRegistry.inspect_model_cls("Qwen3ForCausalLM")
        return
    except ValueError:
        pass
