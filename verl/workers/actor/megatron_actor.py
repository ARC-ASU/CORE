...
class MegatronPPOActor(BasePPOActor):
    ...
    def update_policy(self, dataloader: Iterable[DataProto]) -> Dict:
        ...
        for data in dataloader:
            ...
            # Optional: KL integration (placeholder logging)
            kl_items = data.meta_info.get('kl_items', None) if hasattr(data, 'meta_info') and data.meta_info else None
            if kl_items is not None:
                # We log availability; full token-level KL with Megatron pipeline needs a dedicated implementation.
                # This hook ensures metrics visibility to confirm data flow end-to-end.
                try:
                    metrics['actor/kl_distill_items'] = metrics.get('actor/kl_distill_items', 0) + len(kl_items)
                except Exception:
                    pass
