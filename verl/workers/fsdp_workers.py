...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        super().__init__()
        self.config = config
        self.role_name = kwargs.get('role_name', 'actor_rollout_ref')
    ...
    def _build_model_optimizer(self,
        ...
        # transformers>=4.56 enforces torch>=2.6 inside check_torch_load_is_safe; patch to allow legacy torch
        try:
        ...
        with init_context(), warnings.catch_warnings():
            ...
            # Monkeypatch to allow loading non-safetensors models
            import os
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, remove_previous_ckpt=False, inference_only=False):
        ...
        # Only save full training checkpoint (with optimizer states) if not inference_only
        ...
        # Save HuggingFace format weights for inference
        import torch.distributed
        ...
        if self.rank == 0:
            print(f'Saving actor checkpoint to {local_path}' + (' (inference only)' if inference_only else ''))
