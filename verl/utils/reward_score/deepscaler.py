...
def compute_score(data_source, solution_str, ground_truth, extra_info=None, use_think=False, return_extracted_answer=False):
    ...
    if use_think is False:
        ...
        return (0., "") if return_extracted_answer else 0.
    
    # For math MCQs, use is_mcq=True to get proper A/B/C/D extraction
    model_answer = extract_answer(model_solution, is_mcq=True)
    if model_answer is None:
        return (0., "") if return_extracted_answer else 0.

    # Ensure ground_truths is always properly defined
    if isinstance(ground_truth, (str, float, int)):
        ground_truths = [ground_truth]
    elif isinstance(ground_truth, list):
        ground_truths = ground_truth
    else:
        # For other iterable types (e.g. numpy array), convert to list
        try:
            import numpy as np
            ...
            # If conversion to list fails, treat as a single value
            ground_truths = [ground_truth]
    ...
    for truth in ground_truths:
        truth = str(truth)
        if "\\boxed" in truth:
            processed_truth = extract_answer(truth, is_mcq=True)
    ...
    if not processed_ground_truths:
        return (0., str(model_answer)) if return_extracted_answer else 0.
        
    for ground_truth in processed_ground_truths:
        is_correct = grade_answer_mathd(model_answer, ground_truth) or grade_answer_sympy(model_answer, ground_truth)
        if is_correct:
            return (1., str(model_answer)) if return_extracted_answer else 1.
