...
"""
Merge verl FSDP sharded checkpoints into a HuggingFace model.

After RL training, verl saves sharded DTensor checkpoints per rank.
This script merges them into a single HuggingFace model that can be
loaded with AutoModelForCausalLM.from_pretrained().

Usage:
    python scripts/model_merger.py --local_dir outputs/core_cr/global_step_24/actor

The merged model is written to <local_dir>/huggingface/.
"""
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Merge verl FSDP checkpoints into HuggingFace format")
    parser.add_argument('--local_dir', required=True, type=str,
                        help="Path to the actor checkpoint directory (e.g., outputs/core_cr/global_step_24/actor)")
    parser.add_argument("--hf_upload_path", default=False, type=str,
                        help="Optional HuggingFace Hub repo to upload the merged model")
    args = parser.parse_args()

    assert not args.local_dir.endswith("huggingface"), "The local_dir should not end with huggingface"
    local_dir = args.local_dir

    # Find the world size from rank 0 checkpoint filename
    rank = 0
    world_size = 0
